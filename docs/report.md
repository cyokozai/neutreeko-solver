...
ここまでの検査は、どれも同じ作者が同じ盤の表現の上に書いたものである。盤の表現やルールの読み違いそのものが共通していれば、すべての検査を素通りしうる。そこで、本体のパッケージを使わずにルールを一から実装した検証器 `verify/Oracle.jl` を別に用意した。盤は 25 要素の配列で持ち、勝ち判定も座標の差から直接行うなど、あえて本体と違う書き方をしてある。両者が共有するのは第 2 節の局面文字列だけである。

検証器は深さ制限つきの αβ 探索（反復深化）で「何手以内に勝てるか・負けるか」を求める。探索は履歴を持たない。第 5 節で見たとおり、勝ち負けの距離は 3 回反復の規定に左右されないからである。判定の規則は次のとおり。

-	表が勝ち・負けで距離 d が探索の深さ以下なら、検証器も同じ値・同じ距離を返すこと。反復深化なので、これは「d − 1 手以内では決着しない」ことも含む。
-	表が引き分け、または距離が探索の深さを超えるなら、検証器はその深さ以内で決着を見つけないこと。

突き合わせは `scripts/crosscheck.jl` が行い、結果の全文は `results/crosscheck.md` にある。

| 検査                                                                                                             | 問い合わせ | 不一致 |
|------------------------------------------------------------------------------------------------------------------|-----------:|-------:|
| (a) 層別無作為抽出: 距離 0〜13 の各層 200 件、距離 14〜51 の各層最大 50 件、引き分け 2,000 件（深さ 13）         |      6,640 |      0 |
| (b) 初期局面の 14 手: 各初手の後の局面を、深さ = 表の距離 と 距離 − 1 の 2 回（引き分けの手と初期局面は深さ 20） |         27 |      0 |
| (c) 有効な全局面 × 黒番・白番の両方（深さ 5）。距離 5 以下の 4,773,840 局面は値と距離まで照合                    |  6,936,160 |      0 |
| 補足: 表を TSV に書き出し、検証器のコマンドに直接流す経路（無作為 20,000 件、深さ 9）                            |     20,000 |      0 |

**問い合わせ計 6,942,827 回、不一致 0 件。** 抽出した 6,640 局面では、局面文字列の読み書きと合法手の集合も両実装で一致した。

(b) の中身を一つ挙げる。表によれば、黒の初手 b1-b4 の後の白番局面は「白の勝ち・21 ply」である。検証器は深さ 21 で同じ「勝ち・21 ply」を返し、深さ 20 では決着を見つけなかった（それぞれ 12.6 秒と 9.1 秒）。後ろ向きに 1 秒で求めた数値を、前向きの探索が別経路で一手の狂いもなく再現している。

検査 (c) の深さは 5 にとどまる。深い層は (a) の抽出と (b) の 22 ply に頼っており、たとえば距離 51 の 8 局面について、決着の手数そのものを前向きの探索で確かめてはいない（深さ 13 で決着しないことだけを確かめた）。その深さを保証しているのは、前の小節までの局所整合と不動点反復との全局面一致である。二つの手法はそれぞれ得意な範囲が違い、両方を合わせて全体を覆っている。
...
講義の意味でのエージェントは、環境（盤面と手番）を知覚し、行為（指し手）を選んで環境に返すものである。強解決の表は、それ自体はエージェントではないが、最善の行為を即座に返せる知識である。`src/agent.jl` に、共通の窓口 `choose_move(agent, pos, history)` を持つ三つのエージェントを用意した。

| エージェント            | 知識       | 手の選び方                                                                                                                                   |
|-------------------------|------------|----------------------------------------------------------------------------------------------------------------------------------------------|
| `PerfectAgent(table)`   | 強解決の表 | `best_moves` の最善手（勝ちは最短、負けは最長の粘り、引き分けは引き分けを保つ手）。候補が複数なら乱数（seed 指定可）、乱数を与えなければ先頭 |
| `AlphaBetaAgent(depth)` | なし       | 深さ制限つき αβ 探索。読み切れない末端は評価関数で見積もる                                                                                   |
| `RandomAgent(rng)`      | なし       | 合法手から一様に選ぶ                                                                                                                         |

`AlphaBetaAgent` の評価関数 `default_eval` は、手番側から見て次の 3 項の和である。

-	並びかけ（2 駒が乗り、残り 1 マスが空いている勝ち筋）の数の差 × 10
-	相手の 1 手勝ちの数 × −40
-	中央 3×3（b2〜d4）にある駒の数の差 × 2

決着は ±(1,000,000 − 手数) と評価するので、早い勝ちと遅い負けが好まれる。探索は履歴を見ないので、同一局面 3 回の規定は対局を進める側（`play_game`）だけが扱う。

### 総当たりの結果

`scripts/tournament.jl --games 50 --seed 1` の結果を示す。各組は先手 50 局・後手 50 局の計 100 局で、初期局面から始め、300 ply で打ち切る。セルは行側から見た「勝-分-負」である。

| 行＼列  | perfect | αβ:2    | αβ:4    | αβ:6    | random  | 得点率 |
|---------|---------|---------|---------|---------|---------|-------:|
| perfect | —       | 100-0-0 | 87-13-0 | 100-0-0 | 100-0-0 |  98.4% |
| αβ:2    | 0-0-100 | —       | 0-12-88 | 0-10-90 | 100-0-0 |  27.8% |
| αβ:4    | 0-13-87 | 88-12-0 | —       | 7-54-39 | 100-0-0 |  58.6% |
| αβ:6    | 0-0-100 | 90-10-0 | 39-54-7 | —       | 100-0-0 |  65.2% |
| random  | 0-0-100 | 0-0-100 | 0-0-100 | 0-0-100 | —       |   0.0% |

1 手あたりの思考時間は、`PerfectAgent` が約 0.005 ms（表を 1 回引くだけ）、αβ:4 が約 0.7 ms、αβ:6 が約 5.8 ms だった。

`PerfectAgent` は 400 局で一度も負けていない。これは強解決の定義からの帰結で、表が正しければ、負けの局面から始めない限り負けることはない。初期局面は引き分けなので、負けないことは保証されるが、勝てるかどうかは相手の誤りに依存する。

### 深さを増やしても引き分けに届かない

目を引くのは、`PerfectAgent` から引き分けを取れたのが深さ 4 の 13 局だけで、深さ 6 は 0 局だったことである。深く読むほど強くなる、という素朴な期待とは逆向きに見える。

原因は初手にある。黒番の初期局面で seed を変えて 100 回ずつ初手を選ばせると、深さ 4 は 100 回とも引き分けを保つ b1-c1 / d1-c1 を選び、深さ 2 と深さ 6 は 100 回とも b1-b4 / d1-d4 を選んだ。表によれば b1-b4 は、白が正しく応じれば初期局面から 22 ply で黒が負ける手である。

22 ply 先の負けは、深さ 6 の探索からは見えない。探索が読み切れる範囲の外で決着する誤りは、評価関数の良し悪しでしか避けられない。深さ 4 が引き分けの手を選んだのも、読みの深さで見抜いたのではなく、評価関数の見積もりがたまたまそちらを好んだ結果である。第 7 節で見たとおり、この局面で引き分けを保つ手は 14 手中 2 手しかなく、その後も 15 ply 目まで各局面に 1 手しかない。浅い読みと簡単な評価関数でこの細い道をたどり続けるのは難しい。

強解決の表は、この比較そのものも支えている。表があるので、αβ エージェントの一手一手が最善手だったかどうかを局面ごとに判定でき、「どの深さのエージェントが、何手目で、どれだけ長い負け筋に踏み込んだか」まで数えられる。

### 追試: seed を変えても逆転は残る

上の観測が seed 1 の偶然でないことを確かめるため、`PerfectAgent` 対 αβ:4 と `PerfectAgent` 対 αβ:6 を、seed 1〜5 のそれぞれで 100 局ずつ指させた（`PerfectAgent` から見た勝-分-負）。
...
αβ:4 はどの seed でも 1 割強の引き分けを取り、αβ:6 は 500 局で一度も取れなかった（総当たりの表の 87-13-0 と seed 1 の 90-10-0 が違うのは、各局の乱数が総当たりに参加するエージェントの数に依存するためである）。

深さごとの初手（黒番の初期局面、seed 100 通り。深さ 8 のみ 20 通り）と、初手ごとのルートでの評価値を並べると、理由が見える。

| 深さ | 選んだ初手                                           |
|------|------------------------------------------------------|
| 2, 3 | b1-b4 / d1-d4（100%）                                |
| 4    | b1-c1 / d1-c1（100%）                                |
| 5    | b1-b4 / d1-d4 52%、b1-c1 / d1-c1 48%（評価値が同点） |
| 6    | b1-b4 / d1-d4（100%）                                |
| 8    | b1-c1 / d1-c1（100%）                                |

| 初手               | 表の値        | αβ:4         | αβ:5         | αβ:6         | αβ:8         |
|--------------------|---------------|-------------:|-------------:|-------------:|-------------:|
| b1-c1 / d1-c1      | 引き分け      |       **−4** |       **12** |          −50 |      **−10** |
| b1-b4 / d1-d4      | 22 ply で負け |          −26 |       **12** |      **−10** |          −28 |
| b1-a2 / d1-e2      | 10 ply で負け |          −38 |          −20 |          −50 |          −48 |
| b1-a1 / d1-e1      | 6 ply で負け  |          −48 | 6 ply で負け | 6 ply で負け | 6 ply で負け |
| c4-c5 / c4-c3      | 4 ply で負け  | 4 ply で負け |         同左 |         同左 |         同左 |
| c4 の横・斜め 4 手 | 2 ply で負け  | 2 ply で負け |         同左 |         同左 |         同左 |

（太字はその深さで最も高い評価値。数値は評価関数の単位）

探索は、読みの範囲に収まる負けを正しく見抜いている。2 ply と 4 ply の負けはどの深さでも、6 ply の負けは深さ 5 以上で「負け」と判定された。見抜けないのは 22 ply 先の負けだけで、b1-b4 と b1-c1 の優劣は、探索の末端での評価関数の見積もり（b4 と c4 が 4 段目に並ぶと「並びかけ」の点が入る、など）だけで決まる。その見積もりは深さによって入れ替わり、深さ 4 は c1、5 は同点、6 は b4、8 は再び c1 を選ぶ。

深く読めば単調に良くなる、とは言えない。読みの範囲の外で決着する誤りは、どの深さでも評価関数の当たり外れに委ねられる。22 ply 先の負けを負けと言い切れるのは、全局面を解いた表だけである。
